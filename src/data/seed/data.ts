export const products = [
  {
    id: 'c1f72a7c-6c1c-4bcb-85f1-7a1c4f2c8f34',
    name: 'El Señor de los Anillos',
    price: 19.99,
    stock: 10,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716234904/cover-books/o5jxnnjnufzxoakglo0l.jpg',
    description:
      'En un remoto rincón de la Tierra Media, un joven hobbit llamado Frodo Bolsón hereda el Anillo Único y emprende un peligroso viaje para destruirlo en las llamas del Monte del Destino.',
  },
  {
    id: '3a1a15e6-7a8c-47e3-9a6b-0c4b9d1c76f8',
    name: 'El Hobbit',
    price: 29.99,
    stock: 5,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223622/cover-books/h2zowz5v9j1jkowzyxlu.jpg',
    description:
      'Antes de que el Señor de los Anillos comenzara, Bilbo Bolsón, un hobbit tranquilo y aficionado a la pipa, se embarcó en una aventura para recuperar el tesoro de los enanos de las garras del dragón Smaug.',
  },
  {
    id: 'a5a7f6a6-7f1c-4b4b-8f1e-4c1c4f2c8f34',
    name: 'El Silmarillion',
    price: 9.99,
    stock: 20,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223622/cover-books/h2zowz5v9j1jkowzyxlu.jpg',
    description:
      'En este libro, J.R.R. Tolkien explora la historia de la Tierra Media y los eventos que condujeron a la creación del Anillo Único.',
  },
  {
    id: 'd1c72a7c-6c1c-4bcb-85f1-7a1c4f2c8f34',
    name: 'El Retorno del Rey',
    price: 39.99,
    stock: 3,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223232/cover-books/zw7muks3ozluetnvjxsq.jpg',
    description:
      'En este tercer libro de la trilogía del Señor de los Anillos, Frodo y Sam llegan a la Tierra de Mordor y se enfrentan a los peligros más grandes de su viaje.',
  },
  {
    id: 'e5e7f6a6-7f1c-4b4b-8f1e-4c1c4f2c8f34',
    name: 'Las Dos Torres',
    price: 14.99,
    stock: 15,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223235/cover-books/hbajsstjz5jnoanyluv1.jpg',
    description:
      'En este segundo libro de la trilogía del Señor de los Anillos, Frodo y sus amigos se enfrentan a los horrores del reino de Mordor y deben encontrar una manera de llevar el Anillo Único a la Tierra de Mordor.',
  },
  {
    id: 'f1f72a7c-6c1c-4bcb-85f1-7a1c4f2c8f34',
    name: 'La Comunidad del Anillo',
    price: 24.99,
    stock: 8,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223233/cover-books/bssgxxq19tvi8kve6zmb.jpg',
    description:
      'En este primer libro de la trilogía del Señor de los Anillos, Bilbo Bolsón entrega el Anillo Único a su sobrino Frodo y comienza su peligroso viaje para destruirlo en las llamas del Monte del Destino.',
  },
  {
    id: 'g5a7f6a6-7f1c-4b4b-8f1e-4c1c4f2c8f34',
    name: 'La Sombra del Pasado',
    price: 7.99,
    stock: 25,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223233/cover-books/oyaoqbqmac7b4qehdlpg.jpg',
    description:
      'En este libro, J.R.R. Tolkien explora la historia de la Tierra Media y los eventos que condujeron a la creación del Anillo Único.',
  },
  {
    id: 'h1c72a7c-6c1c-4bcb-85f1-7a1c4f2c8f34',
    name: 'El Amor y el Destino',
    price: 49.99,
    stock: 2,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223231/cover-books/snlu2lpub6d1m0u5gtdf.jpg',
    description:
      'En este libro, J.R.R. Tolkien explora la historia de la Tierra Media y los eventos que condujeron a la creación del Anillo Único.',
  },
  {
    id: 'i5e7f6a6-7f1c-4b4b-8f1e-4c1c4f2c8f34',
    name: 'El Fin de la Tercera Edad',
    price: 17.99,
    stock: 12,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223229/cover-books/bpko5tutfkvddfjsq1gf.jpg',
    description:
      'En este libro, J.R.R. Tolkien explora la historia de la Tierra Media y los eventos que condujeron a la creación del Anillo Único.',
  },
  {
    id: 'j1f72a7c-6c1c-4bcb-85f1-7a1c4f2c8f34',
    name: 'La Guerra de las Joyas',
    price: 34.99,
    stock: 6,
    image_url:
      'https://res.cloudinary.com/djfxyhasn/image/upload/v1716223229/cover-books/jc2krzbad6gyftug4qnx.jpg',
    description:
      'En este libro, J.R.R. Tolkien explora la historia de la Tierra Media y los eventos que condujeron a la creación del Anillo Único.',
  },
];
